import * as gen from "mobiletto-orm-typedef-gen";
export declare const VOLUME_MOUNT_SOURCE = "source";
export declare const VOLUME_MOUNT_DESTINATION = "destination";
export declare const VOL_TYPE_LOCAL = "local";
export declare const VOL_TYPE_S3 = "s3";
export declare const VOL_TYPE_B2 = "b2";
export declare const DEFAULT_ENCRYPTION_ALGO = "aes-256-cbc";
export declare const VOLUME_TYPEDEF: gen.MobilettoOrmTypeDef;
