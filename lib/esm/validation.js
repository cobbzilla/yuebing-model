...
export const isExactRegexMatch = (value, regex) => {
    const match = value.match(regex);
    return !!(match && match.length === 1 && match[0].length === value.length);
};
export const isValidEmail = (value) => isExactRegexMatch(value, EMAIL_REGEX);
export const NORMALIZE_LOWERCASE = (v) => __awaiter(void 0, void 0, void 0, function* () { return Promise.resolve(`${v ? v : typeof v === "undefined" ? "undefined" : v == null ? "null" : v}`.toLowerCase()); });
...
export const findValidEmails = (muck, splitOn = /[\s,<>]+/g) => {
...
export const isSelfVolume = (volume) => volume &&
    ((typeof volume === "string" && volume === SELF_VOLUME_NAME) ||
        volume.name === SELF_VOLUME_NAME);
